...
class _McpTokenGuard:
    ...
    完全 per-user: 共有トークン(旧 MCP_TOKEN)は廃止。認証は UserSetting.mcp_token のみ。
    ローカル(AUTH_MODE != all かつトークン無し)だけ DEV_OWNER で素通し(開発用)。
    """
    ...
    def _resolve_owner(self, token: str) -> str | None:
        # per-user トークン(UserSetting.mcp_token)だけで識別する
        email = email_for_mcp_token(token) if token else None
        if email:
            return email
        # ローカル開発のみ素通し(本番 AUTH_MODE=all では不可)。共有トークンは無い。
        if get_auth_mode() != "all" and not token:
            return DEV_OWNER
        return None
