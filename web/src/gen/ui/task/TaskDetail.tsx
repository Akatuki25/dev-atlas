...
export function TaskDetail({ id }: { id: string }) {
  ...
  return (
    <div style={{ display: "grid", gap: "var(--sp-4)" }}>
      ...
      <KVList items={[{ k: "Id", v: <>{item.id}</> }, { k: "プロジェクトID", v: <>{item.project_id}</> }, { k: "状態", v: <>{item.status}</> }, { k: "説明", v: <>{item.note}</> }, { k: "Created At", v: <>{new Date(item.created_at_unix * 1000).toLocaleDateString()}</> }]} />
